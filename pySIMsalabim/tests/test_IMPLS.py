...
def test_run_IMPLS_simu():
    ...
    ret, mess = run_IMPLS_simu(zimt_device_parameters, session_path, f_min, f_max, f_steps, V, G_frac, GStep, run_mode=False, tVG_name =tVG_name, output_file = 'freqP.dat', tj_name = 'tj.dat', varFile='varFile.dat')
    # Clean up the output
...
def test_IMPLS_parallel():
    """ Test the IMPS_parallel function """
    try:
        if os.path.exists('SIssalabim'):
            cwd = os.path.abspath('.')
        else:
            cwd = os.path.abspath('../..')
        zimt_device_parameters = os.path.join(cwd, 'SIMsalabim','ZimT','simulation_setup.txt')
        ...
        def run(G_frac,ID): # function to run the IMPS simulation in parallel
            ...
            GStep = 0.05
            print('Running')
            ret, mess = run_IMPLS_simu(zimt_device_parameters, session_path, f_min, f_max, f_steps, V, G_frac, GStep, run_mode=False, tVG_name =tVG_name, output_file = 'freqP.dat', tj_name = 'tj.dat', UUID = ID,cmd_pars=cmd_pars,threadsafe=True, varFile='varFile.dat')
